...
class traj:
    ...
    def model(self,post_burnout=True,dt=dt,vector_start_time=10,vector_end_time=120,beta_max=0.2,total_time=total_time):
        ...
        for current_stage in range(0,self.number_of_stages):
            ...
            while self.stage_propellant_masses[current_stage]>0:
                ...
                """if np.abs(self.time[-1]%10)<1:
                    print(self.time[-1],self.orientation[-1])"""
    ...
    def plotter(self, position=True, show_earth=True):
        if position:
            self.fig = plt.figure()
